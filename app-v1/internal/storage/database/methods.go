...
// TODO: refactor this
func (db *db) GetOrdersInLast(duration time.Duration) ([]models.Order, error) {
...
// TODO: refactor this
func (db *db) SaveDataFromOrder(order models.Order) error {
	const op = "db.SaveDataFromOrder"
	...
	err = db.saveOrder(order, tx)
	if err != nil {
		tx.Rollback()
		return fmt.Errorf("%s: %s", op, err)
	}

	err = db.saveDelivery(order, tx)
	if err != nil {
		tx.Rollback()
		return fmt.Errorf("%s: %s", op, err)
	}

	err = db.savePayment(order, tx)
	if err != nil {
		tx.Rollback()
		return fmt.Errorf("%s: %s", op, err)
	}

	err = db.saveItems(order, tx)
	if err != nil {
		tx.Rollback()
		return fmt.Errorf("%s: %s", op, err)
	}
...
func (db *db) saveOrder(order models.Order, tx *sql.Tx) error {
...
func (db *db) saveDelivery(order models.Order, tx *sql.Tx) error {
...
func (db *db) savePayment(order models.Order, tx *sql.Tx) error {
...
func (db *db) saveItems(order models.Order, tx *sql.Tx) error {
